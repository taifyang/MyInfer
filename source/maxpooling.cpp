...
#pragma omp parallel for num_threads(batch)
		for (uint32_t i = 0; i < batch; ++i) 
		{
			...
			if (padding_h_ > 0 || padding_w_ > 0) 
			{
				...
				input_data_->Padding({padding_h_, padding_h_, padding_w_, padding_w_}, std::numeric_limits<int>::lowest());
			}
			else 
			{
			...
			for (uint32_t ic = 0; ic < input_c; ++ic)
			{
				for (uint32_t r = 0; r < input_h - pooling_h + 1; r += stride_h_)
				{
					for (uint32_t c = 0; c < input_w - pooling_w + 1; c += stride_w_)
					{
						...
						Eigen::Tensor<float, 3> region = input_data_->data().slice(offsets, extents);
						output->at(ic, int(r / stride_h_), int(c / stride_w_)) = *std::max_element(region.data(), region.data() + region.size());
					}
				}
			}
	...
	ParseParameterAttrStatus MaxPoolingLayer::GetInstance(const std::shared_ptr<RuntimeOperator> & op, std::shared_ptr<Layer> & max_layer) 
	{
		...
		max_layer = std::make_shared<MaxPoolingLayer>(padding_values.at(0), padding_values.at(1), kernel_values.at(0), 
