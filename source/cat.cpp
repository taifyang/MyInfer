...
namespace my_infer
{
	...
	InferStatus CatLayer::Forward(const std::vector<std::shared_ptr<Tensor<float>>>& inputs, std::vector<std::shared_ptr<Tensor<float>>>& outputs)
	{
		if (inputs.empty()) 
		{
...
#pragma omp parallel for num_threads(outputs.size())
		for (int i = 0; i < outputs.size(); ++i) 
		{
			...
			for (uint32_t j = i; j < inputs.size(); j += output_size) 
			{
				...
				{
					std::copy(input->raw_ptr() + c * input->rows() * input->cols(),
						input->raw_ptr() + (c + 1) * input->rows() * input->cols(),
						output->raw_ptr() + (c + start_channel) * output->rows() * output->cols());
				}
	...
	ParseParameterAttrStatus CatLayer::GetInstance(const std::shared_ptr<RuntimeOperator> & op, std::shared_ptr<Layer> & cat_layer) 
	{
