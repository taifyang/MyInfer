...
namespace my_infer
{
	...
	void ParamLayer::set_weights(const std::vector<float> & weights) 
	{
		...
		for (uint32_t idx = 0; idx < batch_size; ++idx)
		{
			...
			std::vector<float> sub_values{ weights.begin() + start_offset, weights.begin() + end_offset };
	...
	void ParamLayer::set_bias(const std::vector<float> & bias) 
	{
		...
		for (uint32_t idx = 0; idx < batch_size; ++idx) 
		{
			...
			std::vector<float> sub_values{ bias.begin() + start_offset, bias.begin() + end_offset };
