...
my_infer::sftensor PreProcessImage(const cv::Mat& image, const int32_t input_h, const int32_t input_w) 
{
	assert(!image.empty());
...
{
	float gain = min((float)img_shape.height / (float)img_origin_shape.height, (float)img_shape.width / (float)img_origin_shape.width);
	...
	coords.x = max(0, min(coords.x, img_origin_shape.width));
	coords.y = max(0, min(coords.y, img_origin_shape.height));
	coords.width = max(0, min(coords.width, img_origin_shape.width)); 
	coords.height = max(0, min(coords.height, img_origin_shape.height));
}
	...
	const float iou_thresh = 0.25f) 
{
	...
	for (int i = 0; i < outputs.size(); ++i) 
	{
		...
		for (const auto& detection : detections) 
		{
			...
			cv::putText(image, std::to_string(detection.class_id), cv::Point(detection.box.x, detection.box.y), font_face, font_scale, cv::Scalar(255, 255, 0), 4);
		}
		cv::imwrite(std::string("./tmp/output") + std::to_string(i) + ".jpg", image);
	}
}

TEST(test_net, forward_yolo) 
{
	using namespace my_infer;
	RuntimeGraph graph("./tmp/yolov5n.pnnx.param", "./tmp/yolov5n.pnnx.bin");
	...
	for (int i = 0; i < batch_size; ++i) 
	{
		...
		for (int r = 0; r < output1->rows(); ++r) 
		{
			for (int c = 0; c < output1->cols(); ++c) 
			{
				ASSERT_LE(std::abs(output1->at(0, r, c) - output2->at(0, r, c)), 0.05) << " row: " << r << " col: " << c;
			}
		}
	}
}
...
TEST(test_model, yolo_demo) 
{
	...
	for (uint32_t i = 0; i < batch_size; ++i) 
	{
		const std::string& image_path = "./tmp/bus.jpg";  
		image_paths.push_back(image_path);
	}

	const std::string& param_path = "./tmp/yolov5s_batch8.pnnx.param";
	const std::string& bin_path = "./tmp/yolov5s_batch8.pnnx.bin";
