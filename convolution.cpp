...
namespace my_infer 
{
	...
	InferStatus ConvolutionLayer::Forward(const std::vector<std::shared_ptr<Tensor<float>>>& inputs, std::vector<std::shared_ptr<Tensor<float>>>& outputs)
	{
		...
		//{
		//	std::cout << "show weight " << i << ":" << std::endl;
		//	weights_[i]->Show();
		//}
...
#pragma omp parallel for num_threads(batch_size)
		for (uint32_t i = 0; i < batch_size; ++i) 
		{
			...
			for (uint32_t g = 0; g < groups_; ++g)
			{
				...
				std::vector<Eigen::MatrixXf> kernel_matrix_arr(kernel_count_group);//3
				// 展开之后的卷积核
				Eigen::MatrixXf kernel_matrix_c(1, row_len * kernel_c); //9*3

				for (uint32_t k = 0; k < kernel_count_group; ++k) //3
				{
					const std::shared_ptr<Tensor<float>>& kernel = this->weights_.at(k + g * kernel_count_group);
					for (uint32_t ic = 0; ic < kernel->channels(); ++ic) //3
					{
						for (uint32_t r = 0; r < kernel->rows(); r++) //3
						{
							for (uint32_t c = 0; c < kernel->cols(); c++) //3
							{	
								//kernel_matrix_c(0, (ic * kernel->rows() + c) * kernel->cols() + r) = kernel->at(0, r, c);  //(2*3+2)*3+2
								kernel_matrix_c(0, (ic* kernel->rows() + c)* kernel->cols() + r) = kernel->at(ic, r, c);
							}
						}
					}
					//std::cout << kernel_matrix_c << std::endl;
					kernel_matrix_arr.at(k) = kernel_matrix_c;
				}

				Eigen::MatrixXf input_matrix = Eigen::MatrixXf::Zero(input_c_group * row_len, col_len);//3*49 12544
				for (uint32_t ic = 0; ic < input_c_group; ++ic)
				{
					Eigen::Tensor<float, 3> input_channel = input_->slice(ic + g * input_c_group);
					for (uint32_t r = 0; r < row_len; r++)	//9
					{
						for (uint32_t w = 0; w < input_w - kernel_w + 1; w += stride_w_)//230-7+1
						{
							int w_ = w / stride_w_;
							...
							{
								int h_ = h/ stride_h_;
								//input_matrix(ic* row_len + r, c) = input_channel(r % kernel_h + h, r / kernel_h + w, 0); //0-8 9-17 18-26
				...
				outputs.at(i) = output_tensor; //raw_shapes 64*112*112
			}
		}
